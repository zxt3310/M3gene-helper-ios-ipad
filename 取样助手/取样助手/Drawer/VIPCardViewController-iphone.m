...
@interface contentViewPhone : UIView
@property (nonatomic) NSString *titleStr;
@end
@implementation contentViewPhone
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        UITextField *titleTag = [[UITextField alloc] initWithFrame:CGRectMake(19 * iphone_size_W, 20* iphone_size_H , 15, 15)];
        titleTag.enabled = NO;
        titleTag.layer.cornerRadius = 7.5;
        titleTag.backgroundColor = [UIColor colorWithMyNeed:88 green:207 blue:225 alpha:1];
        [self addSubview:titleTag];
        
        UILabel *title = [[UILabel alloc] initWithFrame:CGRectMake(titleTag.frame.origin.x + titleTag.frame.size.width + 18, titleTag.frame.origin.y - 1, 100, 18)];
        title.tag = 1;
        title.text = _titleStr;
        title.font = [UIFont fontWithName:@"STHeitiSC-Light" size:18];
...
@interface contentLbPhone : UILabel
@end
@implementation contentLbPhone
- (instancetype)initWithFrame:(CGRect)frame
{
    self = [super initWithFrame:frame];
    if(self)
...
@interface contentTextFieldPhone : UITextField
@end
@implementation contentTextFieldPhone
- (instancetype)initWithFrame:(CGRect)frame
{
    self = [super initWithFrame:frame];
    if(self)
...
@interface VIPCardViewControllerIphone ()
@end
@implementation VIPCardViewControllerIphone
{
    UIDatePicker *datepicker;
    contentTextFieldPhone *birthTf;
...
- (void)viewDidLoad {
    ...
    contentViewPhone *cardInfoView = [[contentViewPhone alloc] initWithFrame:CGRectMake(0, 64, SCREEN_WEIGHT, 120*iphone_size_H)];
    ...
    contentViewPhone *userInfoView = [[contentViewPhone alloc ] initWithFrame:CGRectMake(0, cardInfoView.frame.origin.y + cardInfoView.frame.size.height + 10, SCREEN_WEIGHT, 268*iphone_size_H)];
...
- (void)collectContentInfo
{
    ...
    for (id object in self.view.subviews) {
        if ([object isKindOfClass:[contentViewPhone class]]) {
            contentViewPhone *content = (contentViewPhone *) object;
            for (id control in content.subviews) {
                if ([control isKindOfClass:[contentTextFieldPhone class]]) {
                    contentTextFieldPhone *tempTf = (contentTextFieldPhone *)control;
